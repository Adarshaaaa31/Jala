const { generatetoken } = require('../config/jwt2')
const user=require('../modles/usermodel2')
const asynchandler=require('express-async-handler')
const { generaterefreshtoken } = require('../config/refreshtoken')
const jwt=require('jsonwebtoken')
const sendEmail = require('./emailctrl2')
...
const userCart=asynchandler(async(req,res)=>{
   ...
   try{
    let products=[]
    console.log("api calkllllllllllllll")
    ...
    for(let i=0;i<cart.length;i++){
        ...
        let getProduct = await Product.findById(cart[i]._id);
        console.log("getProduct",getProduct)
        if (getProduct.quantity < cart[i].quantity) {
            return res.status(400).json({ error: "Not enough quantity available" });
          }
          
             // Update product quantity
             //   console.log("quantityyyy,", getProduct.quantity )

            //   update_quatity = getProduct.quantity - cart[i].quantity;
        getProduct.quantity -= cart[i].quantity;
    //   console.log("quantity,", update_quatity )
...
//         let getProduct = await Product.findById(cart[i]._id);
//         if (getProduct.quantity < cart[i].count) {
//             return res.status(400).json({ error: "Not enough quantity available" });
//           }
//              // Update product quantity
//       getProduct.quantity -= cart[i].count;
//       await getProduct.save();
